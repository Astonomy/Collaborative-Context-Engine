...
As of 2026-08-23, `pnpm audit --prod` reports no known production vulnerabilities. The complete
development graph reports one low-severity `esbuild` advisory
...
when Vitest/Vite's supported range includes it. An unused migration generator that carried a separate
moderate advisory was removed rather than retained as dormant tooling.
